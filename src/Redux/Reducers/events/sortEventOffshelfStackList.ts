...
export default function sortEventOffshelfStackList(store: IStore, action: EventAction) {
  ...
  const list = event.offshelfStackIdList.map(id => ({ id, time: getStackTime(id)(store) }));
  list.sort((a, b) => {
    if (!a.time) return -1;
    if (!b.time) return 1;
    return new Date(b.time).getTime() - new Date(a.time).getTime();
  });
  event.offshelfStackIdList = list.map(item => item.id);
